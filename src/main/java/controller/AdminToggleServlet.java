...
public class AdminToggleServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            if (id.equals(currentAdmin.getEmail())) {
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                response.getWriter().write("자기 자신은 비활성화할 수 없습니다.");
                return;
            }
            
            // ADMIN은 자기 업체 관리자만 비활성화 가능
            if ("ADMIN".equals(currentAdmin.getRole())) {
                ...
                if (targetAdmin == null || !targetAdmin.getCompanyId().equals(currentAdmin.getCompanyId())) {
                    response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    response.getWriter().write("다른 업체의 관리자는 수정할 수 없습니다.");
                    return;
                }
            }
