...
public class LeadWithTrafficDAO {
    ...
    // 유입 로그와 JOIN한 상담 신청 전체 목록 (엑셀 다운로드용, 페이징 없음)
    public List<Map<String, Object>> getAllLeadsWithTraffic(String companyId, String startDate, String endDate) throws SQLException {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT ");
        sql.append("c.id, c.session_id, c.consultation_source, c.name, c.phone, c.email, ");
        sql.append("c.debt_amount, c.monthly_income, c.message, c.status, c.created_at, ");
